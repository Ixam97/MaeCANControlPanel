...
int MCANUpdater::hexFileParser(std::string _file_name)
{
	...
	if (!hexfile.is_open())
	{
		Globals::logError("hexFileParser: Can't open File.");
	...
	while (hexfile.get(ch))
	{
		...
		if (ch == ':') // New Line
		{
			...
			catch (...) { Globals::logError("hexFileParser: Parsing error."); hexfile.close(); return PARSE_ERR_CONVERSION; }
			...
			default: 
				line_checksum = -1;
				Globals::logError("hexFileParser: File is not supported");
			...
			{
				Globals::logError("hexFileParser: Checksum error.");
				hexfile.close();
	...
	hexfile.close();
	Globals::logInfo("hexFileParser: HEX file has been parsed successfull!");
	return 1;
}
...
void MCANUpdater::addFrame(Globals::CanFrame& _frame)
{
	//m_frameInQueue.push(_frame);
	if (m_busy)
	{
		if (Globals::getUidFromData(_frame.data) != m_uid || _frame.resp == 0) {
			m_busy = false;
			Globals::logError("MCANUpdater: Detected conflicting update, abort.");
		...
		case 0x01:
			...
			if (_frame.dlc == 6)
			{
				...
				Globals::logInfo("MCANUpdater: Beginning update in Legacy Mode");
			}
			else if (_frame.dlc == 7)
			{
				...
				{
					m_legacy = false;
					Globals::logInfo("MCANUpdater: Beginning update");
				}
				else
				{
					m_legacy = true;
			...
			if (_frame.dlc == 7)
			{
				...
				else if (_frame.data[6] == 1)
				{
					...
					if (m_page_index == m_file_page_count)
					{
						...
						m_frameOutQueue.push(Globals::CanFrame(0x40, 0, 6, tmp_data));
		...
		if (m_reported_type && m_page_count && m_page_size && !m_compatibility)
		{
			...
			if (return_code == MCAN_UPDATE_IN_PROGRESS) {
				...
				m_frameOutQueue.push(Globals::CanFrame(0x40, 1, 7, tmp_data));
				sendPage(0);
			}
			else
			{
		...
		if (return_code == MCAN_UPDATE_FAILURE_ERROR)
		{
			...
			m_frameOutQueue.push(Globals::CanFrame(0x40, 1, 5, tmp_data));
			Globals::logError("MCANUpdater: Failed to do update.");
		}
		else if (return_code == MCAN_UPDATE_FAILURE_INCOMPATIBLE)
			Globals::logWarn("MCANUpdater: Detected incompatibility between update file and CAN device.");
		else if (return_code == MCAN_UPDATE_SUCCESS)
		{
			Globals::logInfo("MCANUpdater: Update successfull.");
			m_busy = false;
		}
...
void MCANUpdater::sendPage(int _index)
{
	...
	else
	{
		...
		m_frameOutQueue.push(Globals::CanFrame(0x40, 0, 7, tmp_data));
	}
	...
	for (int i = 0; i < m_page_size / 8; i++)
	{
		...
		m_frameOutQueue.push(Globals::CanFrame(0x40, 0, 8, payload_data, 0x300 + i + 1));
	}
...
bool MCANUpdater::getFrame(Globals::CanFrame& _frame)
{
...
int MCANUpdater::startUpdate(uint32_t _uid, uint16_t _type, std::string _file_name)
{
	...
	m_frameOutQueue.push(Globals::CanFrame(0x40, 0, 4, tmp_data));
