        ...
        if (b_draw)
        {
            ...
            if (ImGui::BeginChild("DeviceInfoRegion"))
            {
                if (ConfigWorker::device_list.size() > 0 && ConfigWorker::device_list[current_index].data_complete)
                {
                    ...
                    ImGui::Text(u8"Konfigurationskanäle: %d, Messwertkanäle: %d", ConfigWorker::device_list[current_index].num_config_channels, ConfigWorker::device_list[current_index].num_readings_channels);
                    ...
                    ImGui::SameLine();
                    ButtonDisablable(u8"Aus Liste löschen", true);
