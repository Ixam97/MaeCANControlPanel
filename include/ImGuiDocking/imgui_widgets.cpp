...
bool ImGui::WindowCloseButton(ImGuiID id, const ImVec2& pos)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    // Modified to have rectangular and red close buttons
    // Render
    // FIXME: Clarify this mess
    auto col_backup = g.Style.Colors[ImGuiCol_ButtonHovered];
    g.Style.Colors[ImGuiCol_ButtonHovered] = ImVec4(1, 0, 0, 1);
    ImU32 col = GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
    g.Style.Colors[ImGuiCol_ButtonHovered] = col_backup;
    ImVec2 center = bb.GetCenter();
    if (hovered)
        //window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 12);
        window->DrawList->AddRectFilled(pos + ImVec2(0, g.Style.WindowBorderSize), pos + ImVec2(g.FontSize, g.FontSize) + g.Style.FramePadding * 2.0f, col);
